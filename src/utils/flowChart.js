/*
 * ()
 * Copyright (c) 2013-2020 同方知网技术股股份有限公司
 * 创建人：CNKI-前端开发组
 * 日 期：2017.03.22
...
(function($) {
  ...
  $.lrworkflow = {
    render: function($self) {
      let dfop = $self[0].dfop;
      $self.addClass("lr-workflow");
      // 添加工具栏
      if (!dfop.isPreview) {
        let $tool = $("<div class=\"lr-workflow-tool\" ></div>");
        $tool[0].dfop = dfop;
        ...
        let toolbtnlen = dfop.toolBtns.length;
        if (toolbtnlen > 0) {
          $tool.append("<span></span>");
          for (let i = 0; i < toolbtnlen; ++i) {
            let btn = dfop.toolBtns[i];
      ...
      let $workArea = $("<div class='lr-workflow-workinner' style='width:3000px;height:2000px'></div>")
      ...
      // $self.children(".lr-workflow-work").find(".lr-scroll-box").css({ "width": 3000, "height": 3000 });
      ...
      let $ghost = $("<div class='lr-workflow-rsghost'></div>").attr({
        "unselectable": "on",
        "onselectstart": "return false",
        "onselect": "document.selection.empty()"
      });
      $self.find(".lr-workflow-work").append($ghost);

      let $lineMove = $("<div class=\"lr-workflow-linemover\" style=\"display:none\" ></div>");//操作折线时的移动框
      ...
      $lineMove.on("mousedown", { $workArea: $workArea }, function(e) {
        ...
        let $workArea = e.data.$workArea;

        let ev = $.lrworkflow.mousePosition(e), t = $.lrworkflow.getElCoordinate($workArea[0]);
        let X, Y;
        X = ev.x - t.left;
        Y = ev.y - t.top;
        let p = lm.position();
        let vX = X - p.left, vY = Y - p.top;
        let isMove = false;
        document.onmousemove = function(e) {
          if (!e) e = window.event;
          let ev = $.lrworkflow.mousePosition(e);
          let ps = lm.position();
          X = ev.x - t.left;
          Y = ev.y - t.top;
          if (lm.data("type") == "lr") {
        ...
        document.onmouseup = function(e) {
          let lineId = lm.data("tid");
          let dfop = $("#" + lineId)[0].dfop;
          if (isMove) {
            let p = lm.position();
            if (lm.data("type") == "lr")
      ...
      // 设置线条工具条选定线时显示的操作框
      let $lineOper = $("<div class='lr-workflow-lineoper' style='display:none'><b class='lr'></b><b class='tb'></b><b class='sl'></b><b class='x'></b></div>");
      $workArea.append($lineOper);
      $lineOper.on("click", function(e) {
        if (!e) e = window.event;
        if (e.target.tagName != "A" && e.target.tagName != "B") return;
        let id = $(this).data("tid");
        let type = $(e.target).attr("class");
        if (type == "x") {
    ...
    switchToolBtn: function(dfop, type) {
      let $oldBtn = $("#" + dfop.id + "_btn_" + dfop.currentBtn);
      let $newBtn = $("#" + dfop.id + "_btn_" + type);
    ...
    initDraw: function($workArea, dfop) {
      ...
      $draw.style.width = "3000px";
      $draw.style.height = "2000px";
    },
    ...
    clickWorkArea: function(e) {
      ...
      if (!dfop.isPreview) {
        ...
        $.lrworkflow.addNode($this, dfop, { id: newGuid(), name: name, left: X, top: Y, type: type });
      }
    },
    // 取消所有结点/连线被选定的状态
    blurItem: function(dfop) {
      if (dfop.focusId != "") {
        let $item = $("#" + dfop.focusId);
        if ($item.prop("tagName") == "DIV") {
          ...
          let lineData = $.lrworkflow.getLine(dfop, dfop.focusId);
          ...
          let $lineOper = $(".lr-workflow-lineoper");
          let $lineMove = $(".lr-workflow-linemover");
    ...
    focusItem: function(id) {
      ...
      if ($item.prop("tagName") == "DIV") {
        ...
        let lineData = $.lrworkflow.getLine(dfop, id);
    ...
    addNode: function($workArea, dfop, node, isold) {
      ...
      if (!!node.history && node.history.length > 0) {
        //if (!!node.history[node.history.length - 1].CreateUserName) {
        //    auditor = '(' + node.history[node.history.length - 1].CreateUserName + ')';
        //}
      ...
      if (mark == "conditionnode") {
        ...
        $node = $("<div class=\"lr-workflow-node\" id=\"" + node.id + "\" ><div class=\"lr-workflow-nodeico\"><b class=\"ico_" + node.type + "\"></b></div><div class=\"lr-workflow-nodetext\">" + node.name + auditor + "</div><div class=\"lr-workflow-nodeassemble\" ></div></div>");
      } else {
        node.width = 52;
        node.height = 52;
        if (mark == "startround") {
        ...
        $node = $("<div class=\"lr-workflow-node item-" + mark + "\" id=\"" + node.id + "\" ><div class=\"lr-workflow-nodeico\"></div><div class=\"lr-workflow-nodetext\">" + node.name + auditor + "</div><div class=\"lr-workflow-nodeassemble\" ></div></div>");
      }
    ...
    delNode: function(dfop, nodeData) {
      let tmplines = [];
      for (let i = 0, l = dfop.line.length; i < l; i++) {
        let tmpLine = dfop.line[i];
    ...
    moveNode: function(id, left, top) {
      ...
      let nodedata = $node[0].wfdata;
      let dfop = $node[0].dfop;
      nodedata.left = left;
      nodedata.top = top;
      //重画转换线
    ...
    initNodeEvent: function($workArea) {
      ...
      if (!dfop.isPreview) {
        ...
        $workArea.delegate(".lr-workflow-nodeico", "mousedown", { $workArea: $workArea }, function(e) {
          let $node = $(this).parents(".lr-workflow-node");
          let dfop = $node[0].dfop;
          let nodeData = $node[0].wfdata;
          ...
          let ev = $.lrworkflow.mousePosition(e), t = $.lrworkflow.getElCoordinate($workArea[0]);
          let $ghost = $("#" + dfop.id).find(".lr-workflow-rsghost");
          ...
          X = ev.x - t.left;
          Y = ev.y - t.top;
          let vX = X - nodeData.left, vY = Y - nodeData.top;
          let isMove = false;
          let hack = 1;
        ...
        $workArea.delegate(".lr-workflow-node", "mouseenter", { $workArea: $workArea }, function(e) {
          let dfop = e.data.$workArea[0].dfop;
        ...
        $workArea.delegate(".lr-workflow-nodespot", "mousedown", { $workArea: $workArea }, function(e) {
          ...
          let line = $.lrworkflow.drawLine("1", "lr_workflow_tmp_line", [X, Y], [X, Y], true, true);
          let $draw = $("#" + dfop.id).find("svg");
          $draw.append(line);
        });
        //划线时用的绑定
        $workArea.mousemove(function(e) {
          ...
          let dfop = $workArea[0].dfop;
          if (dfop.currentBtn != "direct") return;
          let lineStart = $workArea.data("lineStart");
          if (!lineStart) return;
          let ev = $.lrworkflow.mousePosition(e), t = $.lrworkflow.getElCoordinate(this);
          let X, Y;
          X = ev.x - t.left;
          Y = ev.y - t.top;
          let line = document.getElementById("lr_workflow_tmp_line");
    ...
    initLineEvent: function($workArea) {
      ...
      $workArea.delegate("g", "dblclick", { $workArea: $workArea }, function(e) {
        let $workArea = e.data.$workArea;
        let dfop = $workArea[0].dfop;
        let lineData = $.lrworkflow.getLine(dfop, this.id);
        let nodeData = null;
        $.each(dfop.node, function(_index, _node) {
          if (_node.id == lineData.from) {
            nodeData = _node;
    ...
    getLineSpotXY: function(nodeId, dfop, type) {
      let nodeData;
      for (let i = 0, l = dfop.node.length; i < l; i++) {
    ...
    addLine: function(dfop, line) {
      ...
      // 获取开始和结束节点的坐标
      let sxy = $.lrworkflow.getLineSpotXY(line.from, dfop, line.sp);
      let exy = $.lrworkflow.getLineSpotXY(line.to, dfop, line.ep);
      line.name = line.name || "";
      line.color = line.color || "1";
      dfop.line.push(line);
      if (line.type && line.type != "sl") {
        let res = $.lrworkflow.calcPolyPoints(sxy, exy, line.type, line.M);
    ...
    resetLines: function(nodeId, dfop) {
      ...
      for (let i = 0, l = dfop.line.length; i < l; i++) {
        ...
        if (line.from == nodeId || line.to == nodeId) {
          ...
          let lineData = $.lrworkflow.getLine(dfop, lineId);
    ...
    setLineType: function(id, newType) {
      ...
      let sxy = $.lrworkflow.getLineSpotXY(from, dfop, lineData.sp);
      let exy = $.lrworkflow.getLineSpotXY(to, dfop, lineData.ep);
    ...
    setLineM: function(id, M, noStack) {
      ...
      let sxy = $.lrworkflow.getLineSpotXY(from, dfop, lineData.sp);
      let exy = $.lrworkflow.getLineSpotXY(to, dfop, lineData.ep);
      let ps = $.lrworkflow.calcPolyPoints(sxy, exy, lineData.type, lineData.M);


      $("#" + id).remove();
      let $line = $.lrworkflow.drawPoly(lineData.color, id, ps.start, ps.m1, ps.m2, ps.end, lineData.marked || dfop.focusId == id);
      let $draw = $("#" + dfop.id).find("svg");
  ...
  $.fn.lrworkflow = function(op) {
    ...
    let dfop = {
      ...
      toolBtns: ["startround", "endround", "stepnode", "confluencenode", "conditionnode", "auditornode", "childwfnode", "systemauditnode"],//"childwfnode"
      nodeRemarks: {
        cursor: "选择指针",
        direct: "步骤连线",
        startround: "开始节点",
  ...
  $.fn.lrworkflowGet = function() {
    ...
    let $self = $(this);
    let $workArea = $self.find(".lr-workflow-workinner");
    let dfop = $workArea[0].dfop;
    ...
    switch (name) {
      ...
      case "set":
        ...
        for (var i = 0, l = op.data.lines.length; i < l; i++) {
          let line = op.data.lines[i];
          $.lrworkflow.addLine(dfop, line);
        }
        break;
    }
  };
  $.fn.lrTemplateworkflowSet = function(name, op) {
    if (document.createElementNS == undefined) {
      return;
    }
    let $self = $(this);
    ...
    switch (name) {
      ...
      case "set":
        var dfop = $workArea[0].dfop;
        for (var i = 0, l = op.userMap.nodes.length; i < l; i++) {
          let node = op.userMap.nodes[i];
          $.lrworkflow.addNode($workArea, dfop, node, true);
        }
        for (var i = 0, l = op.userMap.lines.length; i < l; i++) {
          let line = op.userMap.lines[i];
          $.lrworkflow.addLine(dfop, line);
        }
        break;
    }
